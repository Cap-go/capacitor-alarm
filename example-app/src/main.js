...
const actions = [
  ...
  },
  {
    ...
    inputs: [
      {
        name: 'exactAlarm',
        label: 'Request exact alarm permission (Android)',
        type: 'checkbox',
        value: true,
      },
    ],
  ...
  },
  {
    ...
    label: 'Create alarm',
    description:
      'Attempts to schedule a native alarm. Works on platforms that expose alarm APIs. Returns alarm ID on iOS.',
    inputs: [
      { name: 'hour', label: 'Hour (0-23)', type: 'number', value: 7 },
      { name: 'minute', label: 'Minute (0-59)', type: 'number', value: 30 },
      { name: 'label', label: 'Label', type: 'text', value: 'Wake up' },
      { name: 'skipUi', label: 'Skip UI (Android)', type: 'checkbox', value: false },
      { name: 'vibrate', label: 'Vibrate (Android)', type: 'checkbox', value: true },
    ],
    run: async (values) => {
  ...
  },
  {
    ...
    description:
      'Retrieves a list of alarms scheduled by this app. Works on iOS 26+ with AlarmKit.',
    inputs: [],
    run: async (values) => {
  ...
  },
  {
    ...
    run: async (values) => {
      ...
      if (!alarmId) {
        return {
          success: false,
          message: 'No alarm ID provided. Create an alarm first or enter an ID.',
        };
      }
  ...
  },
  {
    ...
    description:
      'Demonstrates the full alarm lifecycle: create → get (with metadata) → cancel → verify removal.',
    inputs: [
